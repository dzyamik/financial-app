...
const categories = ['Housing', 'Transport', 'Health', 'Food', 'Education', 'Other']
...
async function seed() {
    ...
    } = await (await supabase).auth.admin.listUsers()
    ...
    const { error } = await (await supabase).from('transactions').insert(transactions)
